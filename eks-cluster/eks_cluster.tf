...
resource "aws_eks_node_group" "private-node-group-1-tf" {
  ...
  node_group_name = format("%s-private-ng-1-%s", var.project_prefix, var.build_suffix)
  ...
  tags = {
    Name = format("%s-private-ng-1-%s", var.project_prefix, var.build_suffix)
  }
...
resource "aws_eks_node_group" "private-node-group-2-tf" {
  cluster_name    = aws_eks_cluster.eks-tf.name
  node_group_name = format("%s-private-ng-2-%s", var.project_prefix, var.build_suffix)
  node_role_arn   = aws_iam_role.workernodes.arn
  subnet_ids      = var.private_subnet_ids
  ami_type        = "AL2_x86_64"
  instance_types  = ["t3.medium"]
  ...
  tags = {
    Name = format("%s-private-ng-2-%s", var.project_prefix, var.build_suffix)
  }
